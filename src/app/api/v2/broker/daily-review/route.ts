...
import { generateBrokerDailyReview } from "@/lib/broker-advisor";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const { data, cached } = await generateBrokerDailyReview(ymd, refresh);
    return NextResponse.json({ ok: true, cached, review: data });
  } catch (err) {
    return jsonErrorResponse(err, "生成日终复盘失败");
  }
}
...
export async function POST(req: NextRequest) {
  ...
  const ymd = raw
    ? (parseYmd(String(raw))?.toISOString().slice(0, 10) ?? null)
    : new Date().toISOString().slice(0, 10);
  if (!ymd) {
    return NextResponse.json({ message: "date 须为 YYYY-MM-DD" }, { status: 400 });
  }
  try {
    const { data, cached } = await generateBrokerDailyReview(ymd, true);
    return NextResponse.json({ ok: true, cached, review: data });
  } catch (err) {
    return jsonErrorResponse(err, "生成日终复盘失败");
  }
